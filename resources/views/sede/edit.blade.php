...
@section('content_header')
    <x-page-header 
        ...
        :breadcrumb="[['label' => 'Inicio', 'url' => route('verificarLogin'), 'icon' => 'fa-home'], ['label' => 'Sedes', 'url' => route('sede.index'), 'icon' => 'fa-cog'], ['label' => 'Editar Sede', 'icon' => 'fa-edit', 'active' => true]]"
    />
@endsection
...
@section('content')
    <section class="content mt-4">
        <div class="container-fluid">
            <div class="row">
                <div class="col-12">
                    <a class="btn btn-outline-secondary btn-sm mb-3" href="{{ route('sede.show', $sede->id) }}">
                    ...
                    <div class="card shadow-sm no-hover">
                        ...
                        <div class="card-body">
                            <form method="POST" action="{{ route('sede.update', $sede->id) }}" class="row">
                                @csrf
                                @method('PUT')
                                        ...
                                        <select name="regional_id" 
                                            ...
                                            @foreach ($regionales as $regional)
                                                <option value="{{ $regional->id }}" 
                                                    {{ old('regional_id', $sede->regional_id) == $regional->id ? 'selected' : '' }}>
                                                    {{ $regional->nombre }}
                                                </option>
                                            @endforeach
                                        </select>
                                        ...
                                        <select name="departamento_id" 
                                            ...
                                            @foreach($departamentos as $departamento)
                                                <option value="{{ $departamento->id }}" 
                                                    {{ old('departamento_id', $sede->municipio->departamento_id ?? '') == $departamento->id ? 'selected' : '' }}>
                                        ...
                                        <select name="municipio_id" 
                                            ...
                                            @foreach($municipios as $municipio)
                                                <option value="{{ $municipio->id }}" 
                                                    ...
                                                    {{ old('municipio_id', $sede->municipio_id) == $municipio->id ? 'selected' : '' }}>
                                                    {{ $municipio->municipio }}
                                                </option>
                                            @endforeach
                                        </select>
                                ...
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <select name="status" id="status" class="form-control" required>
                                            <option value="1" {{ $sede->status == 1 ? 'selected' : '' }}>Activo</option>
                                            <option value="0" {{ $sede->status == 0 ? 'selected' : '' }}>Inactivo</option>
                                        </select>
                                    </div>
                                </div>
                                ...
                                <div class="col-12">
                                    ...
                                    <div class="d-flex justify-content-center gap-2">
                                        <a href="{{ route('sede.show', $sede->id) }}" class="btn btn-outline-secondary btn-sm mx-1">
                                            <i class="fas fa-times mr-1"></i> Cancelar
                                        </a>
                                        <button type="submit" class="btn btn-outline-success btn-sm mx-1">
                                            <i class="fas fa-save mr-1"></i> Actualizar Sede
                                        </button>
                                    </div>
                                </div>
                            </form>
...
@section('js')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function filtrarMunicipios() {
                ...
                municipioOptions.forEach(option => {
                    if (option.value === '') {
                        ...
                        option.style.display = 'none';
                        if (option.value === municipioActual) {
            ...
            // Ejecutar al cargar para mostrar los municipios correctos
            filtrarMunicipios();
            
            // Si hay una regional seleccionada al cargar, disparar el cambio
            if (regionalSelect.value) {
                regionalSelect.dispatchEvent(new Event('change'));
            }
        });
    </script>
@endsection
