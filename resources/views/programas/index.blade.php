...
@section('content_header')
    <x-page-header icon="fa-graduation-cap" title="Programas de Formación"
        subtitle="Gestión de programas de formación del SENA" :breadcrumb="[
        ...
        <div class="container-fluid">
            ...
            <div class="row">
                <div class="col-12">
                    ...
                    <x-data-table title="Lista de Programas de Formación" searchable="true"
                        searchAction="{{ route('programa.index') }}" searchPlaceholder="Buscar por código, nombre..."
                        ...
                        <x-slot name="actions">
                            <div class="mr-3">
                                <select id="filterRedConocimiento" class="form-control form-control-sm"
                                    style="width: 150px;">
                            ...
                            <div class="mr-3">
                                <select id="filterNivelFormacion" class="form-control form-control-sm"
                                    style="width: 120px;">
                                    <option value="">Todos los niveles</option>
                                    @foreach(\App\Models\Parametro::whereHas('temas', function ($query) {
                                        $query->where('temas.id', 6); })->get() as $nivel)
                        ...
                        @forelse ($programas as $programa)
                            <tr>
                                ...
                                <td class="px-4">
                                    <div
                                        class="d-inline-block px-3 py-1 rounded-pill {{ $programa->status ? 'bg-success-light text-success' : 'bg-danger-light text-danger' }}">
                                ...
                                <td class="px-4 text-center">
                                    <div class="btn-group">
                                        @can('CAMBIAR ESTADO PROGRAMA DE FORMACION')
                                            <form action="{{ route('programa.cambiarEstado', ['programa' => $programa->id]) }}"
                                                method="POST" class="d-inline">
                                                @csrf
                                                @method('PATCH')
                                                <button type="submit" class="btn btn-light btn-sm" data-toggle="tooltip"
