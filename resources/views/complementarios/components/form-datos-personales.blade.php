...
@php
    ...
    if (!$selectedTipo) {
        if (isset($tiposDocumento) && $tiposDocumento && $tiposDocumento->count() > 0) {
            $cedula = $tiposDocumento->first(function ($t) {
                return stripos($t->name, 'CIUDAD') !== false && stripos($t->name, 'CEDULA') !== false;
            });
            if ($cedula) {
                $selectedTipo = $cedula->id;
            }
        } else {
                    ...
                    @if ($tiposDocumento && $tiposDocumento->count() > 0)
                        @foreach ($tiposDocumento as $tipo)
                            <option value="{{ $tipo->id }}"
                                {{ (string) $selectedTipo === (string) $tipo->id ? 'selected' : '' }}>
                                {{ ucwords(strtolower(str_replace('_', ' ', $tipo->name))) }}
                            </option>
                        @endforeach
                    @else
                        <option value="3" {{ (string) $selectedTipo === '3' ? 'selected' : '' }}>Cédula de
                            Ciudadanía</option>
                        <option value="4" {{ (string) $selectedTipo === '4' ? 'selected' : '' }}>Cédula de
                            Extranjería</option>
                        <option value="5" {{ (string) $selectedTipo === '5' ? 'selected' : '' }}>Pasaporte</option>
                        <option value="6" {{ (string) $selectedTipo === '6' ? 'selected' : '' }}>Tarjeta de
                            Identidad</option>
                        <option value="7" {{ (string) $selectedTipo === '7' ? 'selected' : '' }}>Registro Civil
                        </option>
                        <option value="8" {{ (string) $selectedTipo === '8' ? 'selected' : '' }}>Sin Identificación
                        </option>
                    @endif
                </select>
            </div>
    ...
    <div class="card-body">
        <div class="mb-3">
            ...
            <input type="date" class="form-control" id="fecha_nacimiento" name="fecha_nacimiento"
                value="{{ old('fecha_nacimiento', $userData['fecha_nacimiento'] ?? '') }}"
                ...
                @if ($generos && $generos->count() > 0)
                    @foreach ($generos as $genero)
                        <option value="{{ $genero->id }}"
                            {{ old('genero', $userData['genero'] ?? '') == $genero->id ? 'selected' : '' }}>
                            {{ ucwords(strtolower(str_replace('_', ' ', $genero->name))) }}
                        </option>
                    @endforeach
                @else
                    <option value="9" {{ old('genero', $userData['genero'] ?? '') == '9' ? 'selected' : '' }}>
                        Masculino</option>
                    <option value="10" {{ old('genero', $userData['genero'] ?? '') == '10' ? 'selected' : '' }}>
                        Femenino</option>
                    <option value="11" {{ old('genero', $userData['genero'] ?? '') == '11' ? 'selected' : '' }}>No
                        Define</option>
                @endif
            </select>
        </div>
        ...
        <div class="row">
            <div class="col-md-6 mb-3">
                ...
                <input type="tel" class="form-control" id="telefono" name="telefono"
                    value="{{ old('telefono', $userData['telefono'] ?? '') }}">
            </div>
            <div class="col-md-6 mb-3">
        ...
        <div class="mb-3">
            ...
            <input type="email" class="form-control" id="email" name="email"
                value="{{ old('email', isset($userData['email']) ? strtolower($userData['email']) : '') }}" required>
        </div>
    </div>
</div>
    ...
    <div class="card-body">
                    ...
                    <select class="form-control" id="pais_id" name="pais_id" required>
                        ...
                        @foreach ($paises ?? [] as $pais)
                            <option value="{{ $pais->id }}"
                                {{ old('pais_id', $userData['pais_id'] ?? '') == $pais->id ? 'selected' : '' }}>
                                {{ $pais->pais }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-md-4 mb-3">
                    ...
                    <select class="form-control" id="departamento_id" name="departamento_id" required>
                        ...
                        @foreach ($departamentos ?? [] as $departamento)
                            <option value="{{ $departamento->id }}" { { old('departamento_id',
                                $userData['departamento_id'] ?? '' )==$departamento->id ? 'selected' : '' }}>
                    ...
                    <select class="form-control" id="municipio_id" name="municipio_id" required>
                        ...
                        @if (isset($userData['municipio_id']) && isset($municipios))
                            @foreach ($municipios as $municipio)
                                <option value="{{ $municipio->id }}" { { old('municipio_id', $userData['municipio_id']
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control" id="direccion" name="direccion"
                ...
                placeholder="Use el botón para ingresar una dirección estructurada" readonly required>
            <small class="form-text text-muted">Haga clic en el botón para ingresar una dirección estructurada</small>
        </div>

        <div class="mb-3">
            <button type="button" class="btn btn-primary" id="toggleAddressForm" aria-expanded="false"
                        ...
                        <div class="col-12">
                            <div class="alert alert-info">
                                <small>
                                    ...
                                    <span class="text-muted">Carrera 9A BIS #34-15 Este Apto 301, Barrio
                                        Centro</span><br>
