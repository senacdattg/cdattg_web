...
@section('content_header')
    <x-page-header icon="fa-graduation-cap" title="Resultados de Aprendizaje"
        subtitle="Gestión de resultados de aprendizaje del SENA" :breadcrumb="[['label' => 'Inicio', 'url' => url('/'), 'icon' => 'fa-home'], ['label' => 'Resultados de Aprendizaje', 'icon' => 'fa-graduation-cap', 'active' => true]]" />
@endsection
            ...
            <div class="row">
                <div class="col-12">
                    @can('CREAR RESULTADO APRENDIZAJE')
                        <div class="accordion mb-4" id="accordionCrearResultadoAprendizaje">
                            <div class="card shadow-sm no-hover">
                                <div class="card-header bg-white py-3 d-flex align-items-center"
                                    id="headingCrearResultadoAprendizaje">
                                    <h2 class="mb-0 w-100">
                                        <button
                                            ...
                                            data-target="#collapseCrearResultadoAprendizaje" aria-expanded="false"
                                            aria-controls="collapseCrearResultadoAprendizaje">
                                            <i class="fas fa-plus-circle mr-2"></i> Crear Resultado de Aprendizaje
                                            <i class="fas fa-chevron-down ml-auto"></i>
                                        </button>
                                    </h2>
                                </div>
                                <div id="collapseCrearResultadoAprendizaje" class="collapse"
                                    aria-labelledby="headingCrearResultadoAprendizaje"
                                    data-parent="#accordionCrearResultadoAprendizaje">
                                    <div class="card-body">
                                        @include('resultados_aprendizaje.create')
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <x-data-table title="Lista de Resultados de Aprendizaje" searchable="true"
                        searchAction="{{ route('resultados-aprendizaje.index') }}"
                        searchPlaceholder="Buscar por código, nombre..." searchValue="{{ request('search') }}" :columns="[
                            ['label' => '#', 'width' => '5%'],
                            ...
                            <div class="mr-2">
                                <select id="filterCompetencia" class="form-control form-control-sm" style="width: 200px;">
                                    ...
                                    @foreach($competencias as $competencia)
                                        <option value="{{ $competencia->id }}">{{ Str::limit($competencia->nombre, 25) }}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
                        ...
                        @forelse ($resultadosAprendizaje as $resultado)
                            <tr>
                                ...
                                <td class="px-4">
                                    <div
                                        class="d-inline-block px-3 py-1 rounded-pill {{ $resultado->status == 1 ? 'bg-success-light text-success' : 'bg-danger-light text-danger' }}">
                                ...
                                <td class="px-4 text-center">
                                    <div class="btn-group">
                                        ...
                                        @can('ELIMINAR RESULTADO APRENDIZAJE')
                                            <button type="button" class="btn btn-light btn-sm" data-rap="{{ $resultado->codigo }}"
