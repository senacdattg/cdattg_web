    ...
    if(typeof Swal !== 'undefined') {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                form.method = 'POST';
                form.action = `/inventario/marcas/${id}`;
                form.innerHTML = `
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll('form[action*="marcas"]').forEach(f => {
    ...
    const table = document.querySelector('.marcas-table tbody');
    if(table) {
        const rowsPerPage = 9;
    ...
    window.editMarca = function(id, nombre) {
        ...
        document.getElementById('editMarcaForm').action = `/inventario/marcas/${id}`;
    };

    // Validación en tiempo real
    const inputs = ['create_nombre', 'edit_nombre'];
