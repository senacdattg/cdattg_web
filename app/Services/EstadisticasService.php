...
class EstadisticasService
{
    ...
    public function obtenerDashboardGeneral(bool $usarCache = true): array
    {
        if (!$usarCache) {
            return $this->buildDashboardGeneral();
        }

        return $this->cache('dashboard.general', fn () => $this->buildDashboardGeneral(), 15);
    }

    /**
     * Fuerza la actualización del dashboard y devuelve datos frescos.
     */
    public function refrescarDashboardGeneral(): array
    {
        $this->forgetCache('dashboard.general');

        return $this->obtenerDashboardGeneral(false);
    }
